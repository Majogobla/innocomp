...
function ComputerCard({product})
{
  const { name, image_0, processor, warranty, storage, storage_type, brand, ram, so, price } = product;
    ...
    <div className="w-full overflow-hidden shadow-xl bg-white mx-auto origin-center hover:z-auto transition-transform flex flex-col">
      ...
      <div className='w-full px-4 pb-2 flex flex-col flex-1 justify-between'>
        <div className="border-t-2 border-[#fb5910] px-4 pt-4">
          <div className="font-extralight text-3xl mb-2 text-gray-500 line-clamp-3 hover:line-clamp-none">{name}</div>

          {
            processor ? (<p className='text-[#fb5910] font-bold text-2xl'>Procesador:<span className='text-black font-extralight ml-2'>{processor}</span></p>) : null
          }
          
          <p className='text-[#fb5910] font-bold text-2xl'>Garantía:<span className='text-black font-extralight ml-2'>{warranty}</span></p>

          {
            storage && storage_type ? (<p className='text-[#fb5910] font-bold text-2xl'>Storage:<span className='text-black font-extralight ml-2'>{storage + ' ' + storage_type}</span></p>) : null
          }
          ...
          {
            ram ? (<p className='text-[#fb5910] font-bold text-2xl'>RAM:<span className='text-black font-extralight ml-2'>{ram}</span></p>) : null
          }
          
          {
            so ? (<p className='text-[#fb5910] font-bold text-2xl'>SO:<span className='text-black font-extralight ml-2'>{so}</span></p>) : null
          }
        </div>
