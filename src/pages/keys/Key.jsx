...
function Key({product}) 
{
  ...
  const { nombre, precioV, imagen1 } = product;
  ...
  return (
    <div className="w-full overflow-hidden shadow-xl bg-white mx-auto origin-center hover:z-auto transition-transform flex flex-col rounded-lg">
      <div className=' h-60'>
      ...
      <div className='w-full px-4 py-3 flex flex-col flex-1 justify-between'>
        ...
        <a target='_blank' href={`https://wa.me/50360472885?text=${whatsappMessage}`} className="w-full mt-4 mb-2 flex items-center justify-center gap-2 bg-[#fb5910] hover:bg-[#AD3A05] hover:scale-110 transition-all rounded-xl px-4 py-1 cursor-pointer">
          ...
          <p className="text-2xl font-normal text-white">{precioV}</p>
        </a>
      </div>
    </div>
  )
